...
def _promo_targets(db: Session):
    plans = db.query(SubscriptionPlan).filter(SubscriptionPlan.active.is_(True)).order_by(SubscriptionPlan.price.asc(), SubscriptionPlan.name.asc()).all()
    return [
        {"scope": "ACTIVATION_FEE", "label": "Activation Fee", "plan_id": None},
        {"scope": "ANY_SUBSCRIPTION", "label": "Any Subscription Plan", "plan_id": None},
        *[{"scope": "PLAN", "label": plan.name, "plan_id": plan.id} for plan in plans if plan.name != ACTIVATION_FEE_NAME],
    ]
...
def _validate_promo(db: Session, subscriber_id: int, code: str):
    ...
    discount = original * float(promo.discount_value) / 100.0 if promo.discount_type == "PERCENT" else float(promo.discount_value)
...
def list_promo_codes(db: Session = Depends(get_db), _admin=Depends(require_admin)):
    ...
    return {"status": "success", "promos": [_promo_dict(row, db) for row in rows], "targets": _promo_targets(db)}


@promo_router.post("/admin", status_code=201)
def create_promo_code(data: PromoCreate, db: Session = Depends(get_db), admin: dict = Depends(require_admin)):
    ...
    row = PromoCode(code=code, description=data.description, discount_type=data.discount_type, discount_value=float(data.discount_value), currency=data.currency.upper(), scope=data.scope, target_plan_id=target_plan_id, restricted_email=str(data.restricted_email).lower() if data.restricted_email else None, max_uses=data.max_uses, active=data.active, starts_at=data.starts_at, expires_at=data.expires_at, created_by=str(admin.get("email") or admin.get("sub") or "admin"))
    db.add(row)
    try:
        db.commit()
...
def update_promo_code(promo_id: int, data: PromoUpdate, db: Session = Depends(get_db), _admin=Depends(require_admin)):
...
def quote_promo_code(subscriber_id: int, data: PromoQuote, db: Session = Depends(get_db), _actor=Depends(require_subscriber_or_admin)):
    _, _, plan, promo, original, discount, final = _validate_promo(db, subscriber_id, data.code)
    return {"status": "valid", "promo_code": promo.code, "scope": promo.scope or "ANY_SUBSCRIPTION", "scope_label": _scope_label(promo, db), "plan_id": plan.id, "original_amount": original, "discount_amount": discount, "final_amount": final, "currency": plan.currency}


@promo_router.post("/{subscriber_id}/redeem")
def redeem_promo_code(subscriber_id: int, data: PromoQuote, db: Session = Depends(get_db), _actor=Depends(require_subscriber_or_admin)):
    subscriber, onboarding, plan, promo, original, discount, final = _validate_promo(db, subscriber_id, data.code)
    redemption = PromoRedemption(promo_code_id=promo.id, subscriber_id=subscriber_id, plan_id=plan.id, original_amount=original, discount_amount=discount, final_amount=final, currency=plan.currency.upper(), status="REDEEMED" if final == 0 else "APPLIED")
    db.add(redemption)
    promo.uses_count += 1
    ...
    db.commit()
    return {"status": redemption.status, "promo_code": promo.code, "scope": promo.scope or "ANY_SUBSCRIPTION", "scope_label": _scope_label(promo, db), "original_amount": original, "discount_amount": discount, "final_amount": final, "currency": plan.currency, "payment_waived": final == 0}


@router.post("/{subscriber_id}/order")
def create_binance_order(subscriber_id: int, request: Request, db: Session = Depends(get_db), _actor=Depends(require_subscriber_or_admin)):
    subscriber = db.query(CopySubscriber).filter(CopySubscriber.id == subscriber_id).first()
    if subscriber is None:
        raise HTTPException(status_code=404, detail="Subscriber not found")
    onboarding = get_or_create_onboarding(db, subscriber_id)
    ...
    trade_no = f"BTT{subscriber_id}{int(time.time())}{secrets.randbelow(100000):05d}"[:32]
    public_base = str(request.base_url).rstrip("/")
    description = f"Bethel {plan.name} subscription"
    if charge["activation_fee"] > 0:
        description += f" + one-time activation fee {charge['currency']} {charge['activation_fee']:.2f}"
    payload = {
        "env": {"terminalType": "WEB"},
        "merchantTradeNo": trade_no,
        "fiatAmount": charge["total_amount"],
        "fiatCurrency": charge["currency"],
        "supportPayCurrency": "USDT",
        "description": description,
        "goodsDetails": [{"goodsType": "02", "goodsCategory": "Z000", "referenceGoodsId": f"PLAN{plan.id}", "goodsName": "Bethel Trading Technology Subscription", "goodsDetail": description[:256]}],
    ...
    payment = BinancePayment(subscriber_id=subscriber_id, plan_id=plan.id, merchant_trade_no=trade_no, prepay_id=str(data.get("prepayId") or ""), fiat_amount=charge["total_amount"], fiat_currency=charge["currency"], payment_currency="USDT", status="PENDING", checkout_url=data.get("checkoutUrl") or data.get("universalUrl"))
    db.add(payment)
    ...
    recompute_activation(db, onboarding)
    db.commit()
    return {"merchant_trade_no": trade_no, "status": payment.status, "checkout_url": payment.checkout_url, "deeplink": data.get("deeplink"), "qr_content": data.get("qrContent"), "currency": data.get("currency") or "USDT", "amount": data.get("totalFee"), **charge}


@router.get("/{subscriber_id}/latest")
def latest_payment(subscriber_id: int, db: Session = Depends(get_db), _actor=Depends(require_subscriber_or_admin)):
    payment = db.query(BinancePayment).filter(BinancePayment.subscriber_id == subscriber_id).order_by(BinancePayment.id.desc()).first()
    if payment is None:
        return {"status": "not_found"}
    return {"merchant_trade_no": payment.merchant_trade_no, "status": payment.status, "checkout_url": payment.checkout_url, "currency": payment.payment_currency, "fiat_amount": payment.fiat_amount, "fiat_currency": payment.fiat_currency, "transaction_id": payment.transaction_id}


@router.post("/webhook")
async def binance_webhook(request: Request, binancepay_timestamp: str = Header(default=""), binancepay_nonce: str = Header(default=""), binancepay_signature: str = Header(default=""), db: Session = Depends(get_db)):
    raw_body = await request.body()
    verify_webhook(raw_body, binancepay_timestamp, binancepay_nonce, binancepay_signature)
    try:
    ...
    payment = db.query(BinancePayment).filter(BinancePayment.merchant_trade_no == trade_no).first()
    if payment is None:
