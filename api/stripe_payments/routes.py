...
def create_card_checkout(
    ...
    fields = {
        ...
        "cancel_url": (
            f"{base}/investor-frontend/onboarding.html?payment=stripe-cancelled"
        ),
...
def latest_card_payment(
    ...
    payment = (
        ...
        .filter(StripePayment.subscriber_id == subscriber_id)
...
async def stripe_webhook(
    ...
    payment = (
        db.query(StripePayment)
        .filter(StripePayment.checkout_session_id == session.get("id"))
        .first()
    )
    if payment is None:
