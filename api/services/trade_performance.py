"""Bethel Trading Technologies - signed MT5 trade, ledger and risk analytics."""
...
class TradePerformanceEngine:
    def __init__(self):
        ...
        self.account_number = resolve_active_master_account(self.db)
    ...
    def _funding_base(self) -> float:
        ...
        net_funding = sum(float(item.amount or 0) for item in flows)
        if net_funding > 0:
            return net_funding
        first = (
        ...
        return float(first.balance or first.equity or 0) if first else 0.0
    ...
    def first_trade_at(self):
        ...
        return deal.closed_at if deal else None

    def verified_ledger_history(self) -> Dict:
        """Reconstruct balance only from the active master's signed MT5 ledger.

        The reconstruction is exposed only when it reconciles to the latest MT5
        balance. Equity is never reconstructed because historical floating P/L
        cannot be inferred faithfully without actual equity observations.
        """
        if not self.account_number:
            return {"status": "unavailable", "reason": "no_active_master_account", "balance_history": []}
        ...
        if latest is None:
            return {"status": "unavailable", "reason": "no_equity_snapshots", "master_account": self.account_number, "balance_history": []}
        ...
        for flow in flows:
            if flow.occurred_at is not None:
                events.append({"at": flow.occurred_at, "kind": "cash_flow", "amount": float(flow.amount or 0), "source_id": flow.id})
        for deal in deals:
            if deal.closed_at is not None:
                events.append({"at": deal.closed_at, "kind": "deal", "amount": self._deal_net(deal), "source_id": deal.id})
        events.sort(key=lambda e: (e["at"], 0 if e["kind"] == "cash_flow" else 1, e["source_id"] or 0))
        ...
        if not events:
            return {
                ...
                "equity_snapshot_start_at": first_snapshot.timestamp.isoformat() if first_snapshot else None,
        ...
        history = [{"timestamp": events[0]["at"].isoformat(), "balance": round(balance, 2), "event": "opening_reconstructed_from_verified_ledger"}]
        for event in events:
    ...
    def statistics(self) -> Dict:
        ...
        if len(profits) == 0:
            return {
                ...
                "gross_loss": 0, "profit_factor": 0, "average_win": 0,
                "average_loss": 0, "payoff_ratio": 0, "expectancy": 0,
            }
        ...
        return {
            ...
            "payoff_ratio": round(avg_win / abs(avg_loss), 2) if avg_loss < 0 else 0,
            "expectancy": round(float(profits.mean()), 2),
        }

    def risk_metrics(self) -> Dict:
        profits = self.profits()
        empty = {"sharpe_ratio": 0, "sortino_ratio": 0, "max_drawdown": 0, "value_at_risk_95_amount": 0, "value_at_risk_95_percent": 0}
        ...
        return {
            ...
            "value_at_risk_95_amount": round((var_percent / 100) * self.starting_capital, 2),
