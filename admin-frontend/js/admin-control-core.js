...
async function loadMediaDepartment(){const t=$("#media-library");if(!t)return;try{const d=await apiGet("/broadcast/v1/admin/media"),items=d.items||[];t.innerHTML=items.map(i=>`<tr><td>${i.created_at?new Date(i.created_at).toLocaleString():"—"}</td><td>${escapeHtml(i.title||i.filename)}</td><td>${escapeHtml(i.layout||"—")}</td><td>${stateBadge(i.account_mode||"UNKNOWN")}</td><td><a href="${i.url}" target="_blank" rel="noopener">Open / Download</a>${i.whatsapp_url?`<br><a href="${i.whatsapp_url}" target="_blank" rel="noopener">Send via WhatsApp</a>`:""}</td></tr>`).join("")||'<tr><td colspan="5">No weekly media generated yet.</td></tr>'}catch(e){t.innerHTML=`<tr><td colspan="5">${escapeHtml(e.message||"Media library unavailable")}</td></tr>`}}
