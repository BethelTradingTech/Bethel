...
const titles={customer:"Customer View",overview:"Overview",website:"Website Management",media:"Media Department",investors:"Investor Management",subscribers:"Subscriber Management",operations:"Backup & Security",notifications:"Notifications",legal:"Legal Consent",subscriptions:"Subscription Lifecycle",payments:"Payment Reconciliation",mt5:"MT5 Accounts",terminals:"Master Terminals",analytics:"Performance & Analytics",api:"API & Routes",security:"Security",settings:"System Settings"};
function showView(name){$$(".view").forEach(x=>x.classList.remove("active"));$$(".nav-item").forEach(x=>x.classList.toggle("active",x.dataset.view===name));$("#view-"+name).classList.add("active");$("#page-title").textContent=titles[name];closeMenu();if(name==="api")loadRoutes();if(name==="media")loadMediaDepartment();if(name==="payments")loadPayments();if(name==="subscriptions")loadSubscriptions();if(name==="legal")loadLegalAdmin();if(name==="notifications")loadNotifications();if(name==="operations")loadOperations();if(name==="analytics")loadAnalytics();if(name==="terminals"){loadMasterTerminals();loadPublicMt5Display()}}
...
async function loadMediaDepartment(){const t=$("#media-library");if(!t)return;try{const d=await apiGet("/broadcast/v1/admin/media"),items=d.items||[];t.innerHTML=items.map(i=>`<tr><td>${i.created_at?new Date(i.created_at).toLocaleString():"—"}</td><td>${escapeHtml(i.title||i.filename)}</td><td>${escapeHtml(i.layout||"—")}</td><td>${stateBadge(i.account_mode||"UNKNOWN")}</td><td><a href="${i.url}" target="_blank" rel="noopener">Open / Download</a>${i.whatsapp_url?`<br><a href="${i.whatsapp_url}" target="_blank" rel="noopener">Send via WhatsApp</a>`:""}</td><td>READY FOR UPLOAD</td></tr>`).join("")||'<tr><td colspan="6">No weekly media generated yet.</td></tr>'}catch(e){t.innerHTML=`<tr><td colspan="6">${escapeHtml(e.message||"Media library unavailable")}</td></tr>`}}
async function generateWeeklyMedia(layout){const r=$("#media-result");try{r.textContent="Generating "+layout+" weekly MP4…";const i=await apiPost("/broadcast/v1/admin/media/generate",{layout,duration_seconds:15});r.innerHTML=`Generated <strong>${escapeHtml(i.title||"weekly video")}</strong>. <a href="${i.url}" target="_blank" rel="noopener">Open video</a>${i.whatsapp_url?` · <a href="${i.whatsapp_url}" target="_blank" rel="noopener">Send to WhatsApp</a>`:""}`;await loadMediaDepartment()}catch(e){r.textContent=e.message||"Media generation failed";setStatus(r.textContent,true)}}
async function loadBroadcastEngine(){const s=$("#broadcast-engine-state"),w=$("#broadcast-worker-state"),src=$("#broadcast-source-terminal"),sel=$("#broadcast-terminal");if(!s||!w||!src||!sel)return;try{const [c,x]=await Promise.all([apiGet("/broadcast/v1/admin/control"),apiGet("/connector/v1/status")]);const rows=(x.connectors||[]).filter(r=>!r.subscriber_id);sel.innerHTML=rows.map(r=>`<option value="${r.registry_id}">${escapeHtml(r.label||r.connector_id)} · ${escapeHtml(r.connection_status)}</option>`).join("")||'<option value="">No owner/master terminals</option>';if(c.terminal_registry_id)sel.value=String(c.terminal_registry_id);$("#broadcast-landscape").checked=!!c.landscape_enabled;$("#broadcast-vertical").checked=!!c.vertical_enabled;$("#broadcast-website").checked=!!c.website_enabled;$("#broadcast-youtube").checked=!!c.destinations?.youtube;$("#broadcast-facebook").checked=!!c.destinations?.facebook;$("#broadcast-instagram").checked=!!c.destinations?.instagram;$("#broadcast-tiktok").checked=!!c.destinations?.tiktok;const chosen=rows.find(r=>Number(r.registry_id)===Number(c.terminal_registry_id));s.textContent=c.enabled?"BROADCAST ON":"BROADCAST OFF";w.textContent=c.worker_state||"OFF";src.textContent=chosen?(chosen.label||chosen.account_number):"Not selected";$("#broadcast-start").disabled=!!c.enabled||!rows.length;$("#broadcast-stop").disabled=!c.enabled;const bwe=$("#broadcast-website-enable"),bwd=$("#broadcast-website-disable");if(bwe)bwe.disabled=!c.enabled||!!c.website_enabled;if(bwd)bwd.disabled=!c.enabled||!c.website_enabled;if(c.worker_message)$("#broadcast-engine-result").textContent=c.worker_message}catch(e){s.textContent="UNAVAILABLE";w.textContent="—";src.textContent="—"}}
async function setBroadcastEngine(enabled){try{const v=$("#broadcast-terminal").value;if(enabled&&!v){setStatus("Select an owner/master terminal first",true);return}if(enabled&&!confirm("Start Bethel video generation from sanitized owner/master MT5 telemetry?"))return;await apiRequest("/broadcast/v1/admin/control",{method:"PUT",body:JSON.stringify({enabled,terminal_registry_id:v?Number(v):null,landscape_enabled:$("#broadcast-landscape").checked,vertical_enabled:$("#broadcast-vertical").checked,website_enabled:$("#broadcast-website").checked,youtube_enabled:$("#broadcast-youtube").checked,facebook_enabled:$("#broadcast-facebook").checked,instagram_enabled:$("#broadcast-instagram").checked,tiktok_enabled:$("#broadcast-tiktok").checked,confirm_start:enabled})});setStatus(enabled?"Broadcast engine start requested":"Broadcast engine stop requested");await loadBroadcastEngine()}catch(e){setStatus(e.message||"Unable to update broadcast engine",true)}}
 ...
 try{
  ...
  await apiRequest("/broadcast/v1/admin/control",{
   ...
   body:JSON.stringify({
    ...
    youtube_enabled:$("#broadcast-youtube").checked,
    facebook_enabled:$("#broadcast-facebook").checked,
    instagram_enabled:$("#broadcast-instagram").checked,
    tiktok_enabled:$("#broadcast-tiktok").checked,
    confirm_start:true
   })
  });
...
const mediaLandscape=$("#media-generate-landscape");if(mediaLandscape)mediaLandscape.onclick=()=>generateWeeklyMedia("landscape");const mediaVertical=$("#media-generate-vertical");if(mediaVertical)mediaVertical.onclick=()=>generateWeeklyMedia("vertical");const mediaRefresh=$("#media-refresh");if(mediaRefresh)mediaRefresh.onclick=loadMediaDepartment;
