(function () {
  ...
  function attachLiveStrip(data) {
    ...
    if (!strip) return;
    strip.innerHTML = `<div class="track-live-item"><small>Active Master Return</small><strong>${fmtSignedPercent(data.total_return_percent)}</strong></div><div class="track-live-item"><small>Max Drawdown</small><strong>${fmtPercent(data.maximum_drawdown_percent)}</strong></div><div class="track-live-item"><small>Sharpe</small><strong>${fmtNumber(data.sharpe_ratio)}</strong></div><div class="track-live-item"><small>Sortino</small><strong>${fmtNumber(data.sortino_ratio)}</strong></div><div class="track-live-item"><small>Ledger Status</small><strong>${String(data.verification_status || "READ ONLY")}</strong></div>`;
  }
  ...
  async function load() {
    ...
    buildSkeleton();
    try {
      // Read the active master before and after history. If the owner changes the
      // master during the request window, discard the mixed result and retry on
      // the next short refresh instead of ever showing two accounts as one record.
      const firstSummary = await fetchSummary();
      const historyResponse = await fetch(`${API}/performance/public-history?ts=${Date.now()}`, {cache:"no-store", headers:{Accept:"application/json"}});
      const history = historyResponse.ok ? await historyResponse.json() : {points:[]};
      const data = await fetchSummary();
      if (String(firstSummary.account_number || "") !== String(data.account_number || "")) {
        throw new Error("active master changed during refresh");
      }

      activeMasterMask = data.account_number || null;
      ...
      setText("track-account", `Active master ${data.account_number || "masked"} · ${data.currency || "account currency"}`);
      ...
      setText("track-methodology", data.methodology || "Read-only performance derived from the dynamically resolved active-master history.");
      ...
      if (loadingEl) { loadingEl.className = "track-error"; loadingEl.textContent = "Active-master track record is temporarily unavailable or changing. No stale, fallback or demo figures are displayed."; }
    } finally {
      loading = false;
    }
  }
  ...
  window.addEventListener("load", function () {
    ...
    window.setInterval(guardLiveTelemetry, 3000);
    const accountEl = document.getElementById("public-mt5-account");
    if (accountEl && window.MutationObserver) {
      new MutationObserver(guardLiveTelemetry).observe(accountEl, {childList:true, characterData:true, subtree:true});
    }
  }, {once:true});
})();